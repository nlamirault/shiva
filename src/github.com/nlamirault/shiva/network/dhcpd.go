...
func NewDHCPHandler(serverIP net.IP, startIP net.IP) *DHCPHandler {
	// serverIP := net.IP{172, 30, 0, 1}
	return &DHCPHandler{
		ip:            serverIP,
		leaseDuration: 2 * time.Hour,
		start:         startIP, //net.IP{172, 30, 0, 2},
		leaseRange:    50,
			...
			if leaseNum := dhcp.IPRange(h.start, reqIP) - 1; leaseNum >= 0 && leaseNum < h.leaseRange {
				if l, exists := h.leases[leaseNum]; !exists || l.nic == p.CHAddr().String() {
					...
					return dhcp.ReplyPacket(
						...
						dhcp.ACK,
						h.ip,
						net.IP(options[dhcp.OptionRequestedIPAddress]), h.leaseDuration,
						h.options.SelectOrderOrAll(options[dhcp.OptionParameterRequestList]))
				}
			}
		}
